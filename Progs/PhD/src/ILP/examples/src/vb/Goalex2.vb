...
' ---------------------------------------------------------------------------
'
' Goalex2.vb -- Solving noswot by adding cuts with goals
'
Imports ILOG.CPLEX
...
Public Class Goalex2
   ...
   Friend Class CutGoal
      Inherits Cplex.Goal
      Friend eps As Double = 1E-06
      Friend cut() As IRange
      
      
      Friend Sub New(cuts() As IRange)
         cut = cuts
      End Sub 'New
       
      Public Overrides Function Execute(cplex As Cplex) As Cplex.Goal
         If IsIntegerFeasible() Then
         Return Nothing
         End If 
         Dim num As Integer = cut.Length
         Dim goal As Cplex.Goal = Me
         Dim i As Integer
   ...
   Friend Shared Function MakeCuts(m As IModeler, lp As ILPMatrix) As IRange()
   ...
   Public Overloads Shared Sub Main(ByVal args() As String)
	 ...
	 cplex.SetParam(cplex.IntParam.MIPSearch, cplex.MIPSearch.Traditional)
         If cplex.Solve(New CutGoal(MakeCuts(cplex, lp))) Then
