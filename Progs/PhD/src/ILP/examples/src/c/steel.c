                     ...
                     double *flatinv, double *flatsell, int buildbycolumn)
{
   ...
   /* If an error occurs, the status value indicates the reason for
      ...
      we can't call CPXmsg if CPXopenCPLEX failed.  */
   ...
   *lpstat_p = lpstat;
   ...
   if ( status ) {
      CPXmsg (cpxerror, "CPXgetx failed to get Make solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
   ...
   if ( status ) {
      CPXmsg (cpxerror, "CPXgetx failed to get Inv solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
   ...
   status = CPXgetx (env, lp, flatsell, 2*numprod*tweeks, 
                     3*numprod*tweeks-1);
   if ( status ) {
      CPXmsg (cpxerror, "CPXgetx failed to get Sell solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
          ...
          CPXENVptr env, CPXLPptr lp)
{
   ...
   for (p = 0; p < numprod; p++) {
      ...
      if ( status ) {
         CPXmsg (cpxerror, "%sfor product %d.  Error %d.\n", 
