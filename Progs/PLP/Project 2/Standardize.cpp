...
{int j=1;
 ...
 if(n==1)
	{strcpy(temp, stack.pop());
	 env->setValues(envCount-1, parameter[129].pop(), temp);
	}
 else
 {
 strcpy(temp, stack.pop());
 ...
 if(temp[0]=='(')
 	 while(n--)
		{
	 	 env->setValues(envCount-1, parameter[129].pop(), findTauVal(temp, j++));
		}
 else 
	{//cout << "df";
	 env->setValues(envCount-1, parameter[129].pop(), temp);
   	 n--;
	 while(n--)
		{env->setValues(envCount-1, parameter[129].pop(), stack.pop());
		}
	}
 }
}
				 ...
				 //cout << "in lambda" << endl;
 				 while(parameter[newn].isEmpty()!=1)	//strcmp(delta[i][j].getVal(),0))
					{//env->setValues(envCount-1, parameter[newn].pop(), stack.pop());
					 ...
					 //cout << x << endl;
					 parameter[129].push(parameter[newn].pop());
					 x++;
					}
...
void addParameter(Node *temp, int index)
{
 ...
 if(strcmp(temp->getVal(), " ") && strcmp(temp->getVal(), ","))
