...
bool OrderMaker :: BuildQueryOrderMaker(OrderMaker& sortOrder, 
					OrderMaker& cnfOrder) {
  int sortNum = sortOrder.numAtts;
  int cnfNum = cnfOrder.numAtts;
  int newOrder[100];
  Type newType[100];
  numAtts = sortNum < cnfNum ? sortNum : cnfNum;
  if (numAtts == 0) {
    return false;
  }
  int j;
  for (int i = 0; i < numAtts; i++) {
    for (j = 0; j < cnfNum; j++) {
      if (sortOrder.whichAtts[i] == cnfOrder.whichAtts[j]) {
	newOrder[i] = j;
	newType[i] = sortOrder.whichTypes[j];
	whichAtts[i] = sortOrder.whichAtts[i];
	whichTypes[i] = sortOrder.whichTypes[i];
	break;
      }
    }
    if (j == cnfNum) {
  ...
  for (int i = 0; i < numAtts; i++) {
    cnfOrder.whichAtts[i] = newOrder[i];
    cnfOrder.whichTypes[i] = newType[i];
  }
  return true;
}
...
string OrderMaker :: GetTypes() {
  string result("");
  stringstream ss;
  char buffer[10];
  for (int i = 0; i < numAtts - 1; i++) {
    ss << ToString(whichTypes[i]) << " ";
  }
  ss << ToString(whichTypes[numAtts - 1]);
  result = ss.str();
  cout << "types: " << result << endl;
  return result;
}

Type* OrderMaker :: GetTypesArr() {
  return whichTypes;
}

string OrderMaker :: GetAtts() {
  string result("");
  char buffer[10];
  for (int i = 0; i < numAtts - 1; i++) {
    sprintf(buffer, "%d", whichAtts[i]);
    result += buffer;
    result += " ";
  }
  sprintf(buffer, "%d", whichAtts[numAtts - 1]);
  result += buffer;
  cout << "atts: " << result << endl;
  return result;
}

void OrderMaker :: Init(const int atts[], const Type types[], int cnt) {
  numAtts = cnt;
  for (int i = 0; i < cnt; i++) {
...
Type OrderMaker :: GetType(int x) {
  return whichTypes[x];
}

int OrderMaker :: GetAtt(int x) {
  return whichAtts[x];
}
