...
public:
	...
	void Print (Schema *mySchema, ostream &os = std :: cout);

	bool isNull();

	bool IsEqual(Record* rec);

	int GetNumAtts();

	// sends atts in the form of string seperated by "|"
	void AppendAttsToStringStream(OrderMaker&, stringstream& ss);
};
