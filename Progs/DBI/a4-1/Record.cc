...
bool Record :: IsEqual(Record* rec) {
  if (rec == NULL) {
    return false;
  }
  if (isNull() || rec->isNull()) {
    return false;
  }
  else {
    int size = ((int *) bits)[0];
...
int Record :: GetNumAtts() {
  if (isNull()) {
    ...
    int firstColOff = ((int *) bits)[1];
    return (firstColOff / sizeof(int)) - 1;
  }
}

void Record :: AppendAttsToStringStream(OrderMaker& om, stringstream& ss) {
  int attNum = om.GetAtt(0);
  Type t = om.GetType(0);
  for (int i = 0; i < om.numAtts; i++) {
    char *v = bits + ((int *) bits)[om.whichAtts[i] + 1];
