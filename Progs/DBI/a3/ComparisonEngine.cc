...
int ComparisonEngine :: Compare(Record *left, Record *right, OrderMaker *orderUs) {
	...
	for (int i = 0; i < orderUs->numAtts; i++) {
		...
		switch (orderUs->whichTypes[i]) {
			...
			val1Int = *((int *) val1);
			val2Int = *((int *) val2);
			/*cout << "val1 " << val1Int << endl;
			cout << "val2 " << val2Int << endl;
			cout << "type " << orderUs->whichTypes[i] << endl;
			cout << "left bits " << left_bits << endl;
			cout << "right bits " << right_bits << endl;
			cout << "val1 bits " << val1 << endl;
			cout << "val2 bits " << val2 << endl;
			*/
