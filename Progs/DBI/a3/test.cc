...
void q9() {
  ...
  */
  ...
  Pipe _out2 (pipesz);
  int keepMe2[] = {1};
  int numAttsIn2 = sAtts;
  int numAttsOut2 = 1;
  P_p.Use_n_Pages (buffsz);

  P_p.Run (_ps, _out2, keepMe2, numAttsIn2, numAttsOut2);
    
  Attribute att[] = {IA};
  Schema out_sch2 ("out_sch", numAttsOut2, att);
  cnt = clear_pipe (_out2, &out_sch2, true);

  SF_ps.WaitUntilDone ();
  P_p.WaitUntilDone ();


  cout << "\n\n query8 returned " << cnt << " records \n";
...
void q4 () {
	...
	J.Run (_s, _ps, _s_ps, cnf_p_ps, lit_p_ps,  new Schema (catalog_path, supplier),  new Schema (catalog_path, partsupp));
