...
int Page :: GetNext(Record &fetchme) {
  Record* rec = myRecs->Current(0);
  myRecs->Advance();
  if (rec == NULL) {
    return 0;
  }
...
int Page :: GetRightCount() {
  return myRecs->RightLength();
}

int Page :: GetLeftCount() {
  return myRecs->LeftLength();
}

int Page :: GetNext(Record &fetchme, CNF &cnf, Record &literal) {
  return 1;
}
...
void File :: Open (int fileLen, char *fName) {
	...
	if (fileLen != 0) {
		...
		// cout << "new file read of length " << curLength << endl;
	} else {
		curLength = 0;
	}
}
