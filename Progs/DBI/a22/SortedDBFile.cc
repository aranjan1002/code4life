...
void SortedDBFile :: Copy(HeapDBFile& tempFile) {
  ...
  /*for (int i = 0; i < pageCnt - 1; i++) {
    tempFile.GetPage(&pageToCopy, i);
    myFile->AddPage(pageToCopy, i);
  }
  myFile->setLength(pageCnt);*/
}
...
int SortedDBFile :: GetNext (Record &fetchme, CNF &cnf, Record &literal, Schema* sc) {
  ...
  if (repeat == false && 
    ...
    //cout << "Query order maker built" << endl;
  }
