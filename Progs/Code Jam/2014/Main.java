...
class TaskB {
	...
	for (int i = 0; i < count; i++)
	    answer = Math.max(answer, dfs(i, -1, graph));
	out.printLine("Case #" + testNumber + ":", count - answer);
    }

    private int dfs(int vertex, int last, Graph graph) {
	int best = -1;
	int second = -1;
	for (int i = graph.firstOutbound(vertex); i != -1; i = graph.nextOutbound(i)) {
	    ...
	    if (next == last)
		continue;
	    int current = dfs(next, vertex, graph);
	    if (current > best) {
		second = best;
		best = current;
	    } else
