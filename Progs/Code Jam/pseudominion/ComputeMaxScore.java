...
class ComputeMaxScore {
	...
	try {
	InputStreamReader streamReader = new InputStreamReader(new FileInputStream("input.in"));
	...
	for(int test=1; test<=tests; test++) {
		...
		for(int i=0; i<handCards; i++) {
		    ...
		    String[] x = str.split(" ");
		    cardsInHand.insert(new Card(Integer.parseInt(x[0]), Integer.parseInt(x[1]), Integer.parseInt(x[2])));
		}
	    ...
	    System.out.println("Case #" + test + ": " + calc.maxScore);
	}
	} catch(IOException e) {
