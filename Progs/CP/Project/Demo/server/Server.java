...
public final class Server implements Runnable {
	...
	public void run() {
		try {
			...
			serverSocket = new DatagramSocket(6790);
		...
		for (int i = 0; i < NUMCLIENT; i++) {
			try {
				...
				if (clientInfo.clientType == RegisterClientMessage.
					...
					sendToClient(clientInfo, Integer.toString(ghostClients.size() - 1));
				} else {
					pacMenClients.add(clientInfo);
					sendToClient(clientInfo, Integer.toString(pacMenClients.size() - 1));
				}   
			} catch (Exception e) {
	...
	private void sendToClient(ClientInfo clientInfo, String msg) {
		System.out.println(msg + " sending to client");
	...
	private void broadcast(String boardState) {
		for (ClientInfo clientInfo : pacMenClients) {
			sendToClient(clientInfo, boardState);
		}
